...
public class SwerveTunerOpMode extends CommandOpMode {
    ...
    public void run() {
        ...
        if (USE_GAMEPAD) {
            ...
            robot.drive.swerve.setTargetVelocity(
                    new ChassisSpeeds(vx, vy, omega)
            );
            moduleVelocities = robot.drive.swerve.update();
        }
        ...
        if (!ZEROING) {
            telemetryData.addData("FR kinematics", robot.drive.swerve.getModules()[0].calculateVectorRobotCentric(robot.drive.swerve.getTargetVelocity()).angle());
            telemetryData.addData("FR kinematics", moduleVelocities[0]);
        }
